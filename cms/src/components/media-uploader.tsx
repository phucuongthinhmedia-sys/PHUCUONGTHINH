...
export interface PendingMedia {
  ...
  clientId: string;
  /** Local File object (undefined for social links or already-uploaded items) */
  file?: File;
  /** Remote URL (social link) or already-uploaded public URL */
  url?: string;
...
export interface MediaUploaderProps {
  /** Defined when editing an existing product */
  productId?: string;
  existingMedia?: PendingMedia[];
  onChange: (media: PendingMedia[]) => void;
  /** Product name for "copy to all alt" button */
  productName?: string;
}
...
// ─── setCover helper (exported for testing — Property 2) ─────────────────────

/**
 * Feature: product-form-enhanced, Property 2: Cover image invariant
 * After setCover(list, index), exactly one item has is_cover === true,
 * and that item is at the given index.
 */
  ...
  const addFiles = useCallback(
    (files: FileList | File[]) => {
      ...
      for (const file of arr) {
        ...
        if (!result.valid) {
          // Add as error item so user sees the message
          newItems.push({
            clientId: nextId(),
            file,
            media_type: mediaType,
      ...
      setItems((prev) => {
        // Assign sort_order sequentially after existing items
        ...
        // If no cover exists and we're adding images, set first image as cover
        ...
        <div className="flex gap-2">
          <input
            ...
            className={`flex-1 border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${
              socialError ? "border-red-400" : "border-gray-300"
            }`}
          />
          <button
            type="button"
