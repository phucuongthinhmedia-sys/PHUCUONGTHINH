...
import { ProductCard } from "@/components/import/product-card";
import { EditProductModal } from "@/components/import/edit-product-modal";
...
export default function PreviewPage({ params }: { params: { jobId: string } }) {
  ...
  const {
    ...
    queryKey: ["extracted-products", params.jobId],
    queryFn: () => importService.getExtractedProducts(params.jobId),
  });

  // Auto-select all valid products on load
  useState(() => {
  ...
  const handleBulkImport = async () => {
    ...
    try {
      ...
      // If all succeeded, redirect to products page
      if (result.failed.length === 0) {
        setTimeout(() => {
          router.push("/dashboard/products?filter=draft");
        }, 2000);
      } else {
        // Remove successful products from selection
        setSelectedIds((prev) => {
          const next = new Set(prev);
