...
const STATUS_ICONS = { valid: "✓", warning: "⚠", error: "✕" };
...
export function ImportProductCard({
  product,
  selected,
  onSelect,
  onEdit,
  ...
  return (
    <div
      className={`relative border-2 rounded-lg p-4 transition-all ${selected ? "ring-2 ring-blue-500" : ""} ${STATUS_COLORS[product.validation_status]}`}
    >
      ...
      <div className="absolute top-3 right-3">
        <span
          className={`text-lg ${STATUS_TEXT_COLORS[product.validation_status]}`}
        >
          {STATUS_ICONS[product.validation_status]}
        </span>
      </div>
